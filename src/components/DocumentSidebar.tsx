      ...
      {selectedDocIds.length > 0 && (
        <div className="px-4 py-2 bg-primary/10 border-b border-sidebar-border">
          <p className="text-xs font-medium text-primary">
            {selectedDocIds.length} document{selectedDocIds.length > 1 ? "s" : ""} selected for chat
          </p>
        </div>
      )}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-2 min-w-0 flex-1">
                      {isSelected ? (
                        ...
                        <Square className="w-4 h-4 mt-0.5 flex-shrink-0 text-muted-foreground group-hover:text-sidebar-foreground" />
                      )}
